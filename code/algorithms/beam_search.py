...
def beam_search(d: int, game_number: int, runs: int) -> str:
    ...
    pre: runs is a positive integer that indicates the number of runs to attempt.

    post: returns in a the file path of the saved CSV file containing the solution path 
    if a solution is found within the specified number of runs.
    post: returns None if no solution is found.
    """
    assert isinstance(d, int) and d > 0, "d must be a positive integer."
    assert isinstance(game_number, int) and game_number > 0, "game_number must be higher then zero."
    ...
    assert os.path.isfile(csv_path), f"Invalid game configuration for d={d} and game_number={game_number}"
        ...
        while boards:
            ...
            for current_board in boards:
                if current_board.is_red_car_at_exit():
                    ...
                    csv_name = save_solution_to_csv(solution, d, game_number, beam_width, elapsed_time, heuristic_weight_value, parents)
                    return csv_name
            ...
            boards = []
            # Select the top states according to the beam width for further exploration
...
def calculate_heuristic(board: Board, heuristic_weight_value: float) -> int:
    """
    Calculate the heuristic value for a given board state using a specific heuristic function.

    pre: heuristic_weight_value is a non-negative float representing the weight in the heuristic calculation.
...
def initialize_search(initial_board: Board) -> Tuple[Board, Dict[int, Optional[Tuple[int, str, int]]]]:
    """Initializes the parents dictionary for the search."""
    parents: Dict[int, Optional[Tuple[int, str, int]]] = {}
...
def process_moves(boards: list[Board], parents: dict[int, tuple[int, str, int]]) \
    ...
    post: Returns a list of tuples containing next possible board states, the car id, and the steps taken.
    """
        ...
        for car_id in movable_vehicles:
            for move_direction, step_list in possible_moves[car_id]:
                for steps in step_list:
                    ...
                    # Store new board and heuristic value in list if not already in parents
                    h = hash(new_board) 
                    if h not in parents:
...
def score_states(states: list[tuple[Board, str, int]], heuristic_weight_value: float) -> list[tuple[int, Board, str, int]]:
    """
    Assign heuristic scores to each state based on a specified heuristic function.

    post: returns a list of tuples containing heuristic scores, boards, car ids, and steps.
    """
...
def apply_beam(states: list[tuple[int, Board, str, int]], beam_width: int) -> list[tuple[int, Board, str, int]]:
    """
    Apply beam search selection to retain only the top states based on their heuristic scores.

    pre: beam_width is a positive integer representing the number of top states to retain.
    post: returns a list of tuples containing the top states after beam search selection.
    """
    assert isinstance(beam_width, int) and beam_width > 0, "beam_width must be a positive integer."
...
def reconstruct_path(parents: dict, state: int):
    ...
    post: returns a list of lists representing the sequence of actions to reach from the initial to the goal state.
    """
...
def save_solution_to_csv(solution: List[List[Union[str, int]]], d: int, game_number: int,\
    ...
    filepath2 = f'Experiment/results_bs.csv'
    ...
    with open(filepath2, mode='a', newline='') as file:
        ...
        if not file_exists:
            ...
            file_exists = True
        writer.writerow([game_number, 'bs', len(parents) - 1, elapsed_time, beam_width, len(solution), heuristic_weight_value])
