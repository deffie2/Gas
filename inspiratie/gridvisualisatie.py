# bron = https://www.101computing.net/rush-hour-backtracking-algorithm/
...
def moveCars(grid, cars):
    ...
    for car in cars:
        ...
        if car[3] == True:  # horizontale auto
            if color == 2:  # Red Car Heuristic - Probeer eerst naar rechts te bewegen
                if grid[car[1]][car[2] + car[0]] == 0:
                    # Naar rechts bewegen?
                    grid[car[1]][car[2] + car[0]] = color
                    grid[car[1]][car[2]] = 0
                    car[2] += 1
                    drawGrid(30)  # 30 is de breedte van elk vakje op het rooster
                    myPen.getscreen().update()
                    time.sleep(0.5)  # Voeg vertraging toe
            ...
            else:  # Alle andere horizontale auto's: probeer eerst naar links te bewegen
                if grid[car[1]][car[2] - 1] == 0:
                    # Naar links bewegen?
                    grid[car[1]][car[2] + car[0] - 1] = 0
                    grid[car[1]][car[2] - 1] = color
                ...
                if grid[car[1]][car[2] + car[0]] == 0:
                    ...
                    grid[car[1]][car[2]] = color
                    grid[car[1]][car[2] + car[0]] = 0
        else:  # Verticale auto's...
            # Heuristiek: Trucks (grootte 3) bewegen eerst naar beneden, terwijl auto's (grootte 2) omhoog gaan
            if car[0] == 3:  # Trucks
                ...
                if grid[car[1] - 1][car[2]] == 0:
                    ...
                    car[1] -= 1
                    drawGrid(30)  # 30 is de breedte van elk
                    myPen.getscreen().update()
                    time.sleep(0.5)  # Voeg vertraging toe
                    if moveCars(grid, cars) == True:
                        return True
